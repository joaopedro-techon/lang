...
variable "security_group_default" {
  type = string
  validation {
    condition     = can(regex("^(sg-[0-9a-z-]+)$", var.security_group_default))
    error_message = "Erro ao configurar a 'security_group_default' os valores devem estar configurada corretamente, começando com \"sg-\"."
  }
}
