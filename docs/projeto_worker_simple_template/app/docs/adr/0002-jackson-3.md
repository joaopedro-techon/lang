...
`FAIL_ON_UNKNOWN_PROPERTIES`, `NON_NULL` e afins valiam para o que a aplicação serializava à mão
(o record do Firehose), mas **não para os payloads da fila** — duas stacks JSON com configurações
que podiam divergir em silêncio, a mesma data saindo em formato diferente conforme o destino.
...
- Uma configuração só para SQS, SNS e Firehose.
